...
12. **전투씬 포함 여부 확인** — 포함 시 기풍 선언 준비 (`reference/style-guide-battle.md` 참조). 규모별(소/중/대) 적용 방식 결정
...
- `core-rules.md` 자동 주입됨. 전투씬 포함 시 `reference/style-guide-battle.md` 참조 지시

**에이전트 지시**:
- `scene-writer` Phase 4 (Writing) 비트 구조 준수
- **reference/ 파일 직접 Read 금지** — `core-rules.md` 자동 주입 + 아웃라인에 필요 정보 포함됨. 전투씬은 예외(`style-guide-battle.md` Read 허용)
- 마크다운 형식 금지, 순수 산문
...
**중복 Read 방지**: Phase 1에서 이미 로드한 파일(현재작업상태, 캐릭터, 메인플롯-상세 등)은 Lead가 전달한 컨텍스트 요약을 우선 사용. 직접 Read는 검증이 필요한 설정 파일(타임라인, 역사배경, 과학기술체계)에 한정.
