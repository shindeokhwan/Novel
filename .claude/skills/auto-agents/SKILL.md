...
Sub-agents 7개를 활용한 9단계 파이프라인.

### Phase 0.5: Pre-Flight Check [Lead]

Phase 1 진입 전 필수 사전 점검. 아웃라인 작성 전에 의무 사항을 확정하여 Phase 6 재작업을 방지한다.

1. `reference/ability-upgrade-current.md`의 **최근 기록** 섹션에서 마지막 소(小) 업그레이드 챕터 확인
2. `(현재 챕터 번호) - (마지막 소 업그레이드 챕터)` 계산
   - **4챕터 초과**: 이번 챕터에 소 업그레이드 **필수 배치**. 트랙(AT1-AT4) 균형 확인 → 가장 오래 업그레이드 없던 트랙 우선
   - **3-4챕터**: 권장. 아웃라인에 배치 여부 명시
   - **1-2챕터**: 선택. 자연스러우면 배치
3. 같은 트랙 3회 연속 여부 확인 → 해당 시 다른 트랙으로 강제 전환
4. 결과를 **업그레이드 의무 플래그**로 Phase 2 아웃라인에 전달:
   - `UPGRADE_REQUIRED: true/false`
   - `RECOMMENDED_TRACK: AT1/AT2/AT3/AT4`
   - `RECOMMENDED_REPERTOIRE: [코드]` (예: I05, G06, L03)
...
9. 바둑-전략 대응 현황 확인 (현재 포석/중반전/끝내기 단계)

#### 조건부 로드 (해당 시에만)
10. **전투씬 포함 시에만**: `reference/style-guide-battle.md` — 기풍 선언 준비, 규모별 적용 방식 결정
11. **쾌감 전략 확인이 필요한 챕터만**: `reference/entertainment-density.md` — 축적/해소 위치, 엔딩 방식. 단, `쾌감카운터.md`(6번)에서 E패턴 현황은 이미 확인됨. 규칙 참조가 필요할 때만 로드
12. **구조 결정이 필요한 챕터만**: `reference/plot-structure.md` — 챕터 성격 유형/씬 클라이맥스 위치. 직전 챕터와 동일 유형이 아닌지 확인이 필요할 때만 로드

#### 선언 (매 챕터)
13. **챕터 성격 유형 선언** — 포석형/돌파형/여운형/인간형/전환형 중 하나 명시적 선택
14. **씬 클라이맥스 위치 선언** — 전면배치/중간배치/후면배치 중 하나 선택
15. **엔딩 방식 확인** — 직전 챕터 엔딩 방식 확인, 동일 방식 금지
...
- **`reference/core-rules.md`를 프롬프트에 포함** (Sub-agent 집필 규칙 요약본). 전투씬 포함 시 `reference/style-guide-battle.md` 추가 Read 지시
...
Phase 4B에서 이미 처리된 기계적 스캔(시대착오어, SVO, em-dash, 분리호칭)은 **생략**하고, 심층 항목에 집중한다.
- 대상: Phase 4B 수정 완료된 초고
- **심층 검사 항목**:
  - 대화 자연스러움: 정보전달형 대사 비율, 대화 노이즈 존재, 캐릭터 음성 구분도
  - 갈등 해결 속도: 해결-잔여 긴장 존재, 갈등 깊이 규칙 준수
  - 구조 다양성: 에피소드 구조 패턴, 성장 곡선 비균일성, 비합리적 행동 존재
  - 감각 다양성: 씬당 감각 종류, 동일 조합 연속 여부, 환경 묘사 변주
- 점수 산출 (0-100) + P1/P2 수정 사항 목록
- **4B 잔존 확인**: 기계적 스캔 누락분이 있으면 P1으로 추가 보고
- **7ch 초과 시 분할 필수** (L1)
...
**중복 Read 방지**: Phase 1에서 이미 로드한 파일(현재작업상태, 캐릭터, 메인플롯-상세 등)은 Lead가 전달한 컨텍스트 요약을 우선 사용. 직접 Read는 검증이 필요한 설정 파일(타임라인, 역사배경)에 한정.
- 대상: 초고 + 관련 설정 파일
...
| `reference/core-rules.md` | Phase 3 Sub-agent 프롬프트 주입용 규칙 요약 | Mode A Phase 3 |
